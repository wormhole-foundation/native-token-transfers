...
import { PlatformNativeSigner, encoding } from '@wormhole-foundation/sdk-connect';
...
import { chainToPlatform, Wormhole, chains, assertChain, toUniversal, networks, isNetwork, signSendWait, canonicalAddress } from '@wormhole-foundation/sdk';
import '@wormhole-foundation/sdk-evm-ntt';
import { NTT, SolanaNtt } from '@wormhole-foundation/sdk-solana-ntt';
import '@wormhole-foundation/sdk-definitions-ntt';
import { SolanaAddress } from '@wormhole-foundation/sdk-solana';
import { EvmPlatform } from '@wormhole-foundation/sdk-evm';
import { Wallet, NonceManager, ethers } from 'ethers';
...
class EvmNativeSigner extends PlatformNativeSigner {
    opts;
...
const YARGSCommand = yargs(hideBin(process.argv))
    ...
    const wh = new Wormhole(network, [solana.Platform, evm.Platform], overrides);
    const ch = wh.getChain(chain);
    const [_, ctx, ntt] = await pullChainConfig(network, { chain, address: toUniversal(chain, chainConfig.manager) }, overrides);
...
async function upgradeEvm(pwd, ntt, ctx, signerType, evmVerify) {
    ...
    await withCustomEvmDeployerScript(pwd, async () => {
        execSync(`forge script --via-ir script/DeployWormholeNtt.s.sol \
...
async function deploySolana(pwd, version, mode, ch, token, payer, initialize, managerKeyPath, binaryPath, priorityFee) {
    ...
    const ntt = new SolanaNtt(dummy.network, dummy.chain, dummy.connection, dummy.contracts, version ?? undefined);
    ...
    if (initialize) {
        ...
        const tx = ntt.initialize(toUniversal(ch.chain, payerKeypair.publicKey.toBase58()), {
...
async function pushDeployment(deployment, signerType, evmVerify, yes, filePath) {
    ...
    for (const k of Object.keys(diff)) {
        if (k === "version") {
            ...
            const address = toUniversal(deployment.manager.chain, diff[k]?.push);
...
async function pullDeployments(deployments, network, verbose) {
    ...
    for (const [chain, deployment] of Object.entries(deployments.chains)) {
        ...
        const [remote, ctx, ntt, decimals] = await pullChainConfig(network, { chain, address: toUniversal(chain, managerAddress) }, overrides);
...
export { YARGSCommand, ensureNttRoot };
