...
module ntt_governance::governance_tests {
    ...
    fun test_governance_id_check_correct() {
        ...
        // Construct payload with correct governance ID
        let gov_id_bytes = bytes32::from_address(object::id_address(&gov));
        let mut payload = bytes32::to_bytes(gov_id_bytes);
        // Append some action data
        bytes::push_u8(&mut payload, 42);

        // Should succeed — correct governance ID
        let action_data = governance::test_parse_governance_payload(&gov, payload);
    ...
    fun test_handoff() {
        ...
        // 2. Create gov_b (empty) via init
        scenario.next_tx(gs::admin());
        governance::init_test_only(test_scenario::ctx(&mut scenario));
