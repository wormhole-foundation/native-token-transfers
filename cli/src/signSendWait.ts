...
export const signSendWaitEvmSpecialOwner = async <N extends Network>(
	...
	const executorInterface = new Interface([
	...
	for await (const tx of txs) {
		...
		const newCallData = executorInterface.encodeFunctionData("execute", [
			tx.transaction.to,
			tx.transaction.data,
		]);
...
export const newSignSendWaiter = <N extends Network, C extends Chain>(
	nttOwner: string | undefined,
) => {
	return async (
		chain: ChainContext<N, C>,
		...
		return await signSendWaitWithOverride(chain, xfer, signer, nttOwner);
	};
};
