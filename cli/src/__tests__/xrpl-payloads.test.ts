...
function buildOnboardingHex(
  ...
  return (
    "5852504C" + // "XRPL"
    toHex(decodeAccountID(admin)).substring(2) +
    padHex(toHex("NTT"), { dir: "left", size: 32 }).substring(2) +
    toHex(initialTicket, { size: 8 }).substring(2) +
    toHex(ticketCount, { size: 8 }).substring(2) +
    initDataHex
  );
}
...
describe("parseAdmin — round-trips xrpl_accounts/register_peer.ts encoding", () => {
  it("RegisterPeer (action 0x01)", () => {
    ...
    const hex =
      "5841444D" + // "XADM"
      "01" +
      toHex(decodeAccountID(target)).substring(2) +
      toHex(chainId, { size: 2 }).substring(2) +
      peer;
  ...
  it("RotateAdmin (action 0x02)", () => {
    ...
    const hex =
      ...
      "02" +
      toHex(decodeAccountID(target)).substring(2) +
      toHex(decodeAccountID(newAdmin)).substring(2);
    const parsed = parseAdmin(Buffer.from(hex, "hex"));
