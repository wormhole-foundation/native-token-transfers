...
export const YargsCommand = yargs(hideBin(process.argv))
	...
	.command(
		...
		async (argv) => {
			...
			if (localPath) {
				...
				execSync(`${localPath}/cli/install.sh`);
			} else {
				let branchArg = "";
				let repoArg = "";
				if (argv["branch"]) {
				...
				const installScript =
					"https://raw.githubusercontent.com/wormhole-foundation/example-native-token-transfers/main/cli/install.sh";
	...
	.command(
		...
		async (argv) => {
			...
			execSync(
				`git clone -b main https://github.com/wormhole-foundation/example-native-token-transfers.git ${path}`,
			);
		},
	)
	.command(
	...
	.command(
		...
		async (argv) => {
			...
			console.log(
				`Upgrading ${chain} from version ${currentVersion} to ${toVersion || "local version"}`,
			);
			...
			console.log(
				`Successfully upgraded ${chain} to version ${toVersion || "local version"}`,
			);
		},
	)
	.command(
		...
		async (argv) => {
			...
			for (const [chain, missingConfig] of Object.entries(missing)) {
				...
				for (const relayingTarget of missingConfig.standardRelaying) {
					const tx = (
						(await ntt.getTransceiver(0)) as EvmNttWormholeTranceiver<
							...
							EvmChains
						>
					).setIsWormholeRelayingEnabled(relayingTarget, true);
					await signSendWaitFunc(ctx, tx, signer.signer);
				}
				for (const relayingTarget of missingConfig.specialRelaying) {
					const tx = (
						(await ntt.getTransceiver(0)) as EvmNttWormholeTranceiver<
							...
							EvmChains
						>
					).setIsSpecialRelayingEnabled(relayingTarget, true);
					await signSendWaitFunc(ctx, tx, signer.signer);
				}
				...
				if (missingConfig.solanaUpdateLUT) {
					...
					const solanaNtt = ntt as SolanaNtt<Network, SolanaChains>;
					const tx = solanaNtt.initializeOrUpdateLUT({
						payer: new SolanaAddress(signer.address.address).unwrap(),
						owner: new SolanaAddress(signer.address.address).unwrap(),
					});
					try {
						await signSendWait(ctx, tx, signer.signer);
					} catch (e: any) {
	...
	.command("solana", "Solana commands", (yargs) => {
		yargs
			...
			.command(
				"set-mint-authority <newAuthority>",
						...
						if (multisigInfo.m === 1) {
							...
							for (let i = 0; i < n; ++i) {
								...
								if (
									(
										multisigInfo[
											`signer${i + 1}` as keyof spl.Multisig
										] as PublicKey
...
async function deployEvm<N extends Network, C extends Chain>(
	...
	if (verify) {
		const etherscanApiKey = configuration.get(ch.chain, "scan_api_key", {
			reportError: true,
		});
		if (!etherscanApiKey) {
			process.exit(1);
...
async function deploySolana<N extends Network, C extends SolanaChains>(
	...
	if (mode === "burning") {
		...
		if (actualMintAuthority !== expectedMintAuthority) {
			...
			console.error(
				`spl-token authorize ${token} mint ${expectedMintAuthority}`,
			);
			process.exit(1);
		}
	}
	...
	if (initialize) {
		...
		const signer = await getSigner(
			ch,
			"privateKey",
			base58.encode(payerKeypair.secretKey),
		);
		...
		for (const [toChain, to] of Object.entries(deps)) {
			...
			if (peer === null || peer?.address?.address === undefined) {
				...
				console.log(
					...
					peer.address,
					"manager addr",
					to.manager.address.toUniversalAddress(),
				);
				if (
					!peer.address.address.equals(to.manager.address.toUniversalAddress())
				) {
...
async function checkSolanaBinary(
	...
	const providedProgramIdHex = new PublicKey(providedProgramId)
		.toBuffer()
		.toString("hex");
