    ...
    (define-public (send-token-transfer (transceiver <transceiver-trait>) (amount uint) (recipient-chain (buff 2)) (recipient-address (buff 32)))
      (let ((check1 (try! (check-enabled)))
            ...
            (token-decimals (contract-call? .token-owner-8742 get-decimals))
            ...
            (ntt-peer (unwrap! (contract-call? .ntt-manager-state-8742 peers-get recipient-chain) ERR_UNKNOWN_CHAIN))
        ...
        (try! (contract-call? .token-owner-8742 lock-or-burn-tokens amount tx-sender))
    ...
    (define-public (receive-token-transfer (source-chain (buff 2)) (source-ntt-manager (buff 32)) (ntt-manager-payload (buff 1024)))
      (let ((check1 (try! (check-enabled)))
            ...
            (native-decimals (contract-call? .token-owner-8742 get-decimals))
            (amount (try! (scale-decimals (get amount ntt-manager-message) peer-decimals native-decimals)))
            (peer-data (unwrap! (contract-call? .ntt-manager-state-8742 peers-get source-chain) ERR_UNKNOWN_CHAIN))
        ...
        (try! (contract-call? .ntt-manager-state-8742 consume-message uid))
        (match recipient
          ;; We know recipient account, unlock and send to account
          r (try! (contract-call? .token-owner-8742 unlock-or-mint-tokens amount r))
          ;; We don't know recipient account yet. Allow account to claim later
          (let ((idx {
                  protocol: protocol,
                  addr32: recipient-addr32
                })
                (tokens-pending (default-to u0 (contract-call? .ntt-manager-state-8742 tokens-pending-get idx))))
            (try! (contract-call? .ntt-manager-state-8742 tokens-pending-set idx (+ tokens-pending amount)))))
        (ok {
    ...
    (define-public (release-tokens-pending (transceiver <transceiver-trait>) (recipient principal))
      (let ((check1 (try! (check-enabled)))
            ...
            (amount (unwrap! (contract-call? .ntt-manager-state-8742 tokens-pending-get idx) ERR_NO_TOKENS_PENDING)))
        (try! (contract-call? .token-owner-8742 unlock-or-mint-tokens amount recipient))
        (try! (contract-call? .ntt-manager-state-8742 tokens-pending-delete idx))
        (ok amount)))
    ...
    (define-public (initialize-locking-mode (contract principal))
      (begin
        ...
        (contract-call? .token-owner-8742 initialize-locking-mode contract)))
    
    (define-public (initialize-burning-mode (decimals uint))
      (begin
        (try! (check-uninitialized))
        (var-set initialized true)
        (contract-call? .token-owner-8742 initialize-burning-mode decimals)))
    ...
    (define-public (initialize-from-previous
      ...
      (let ((active-contract (contract-call? .ntt-manager-state-8742 get-owner)))
        (try! (check-uninitialized))
        (asserts! (is-eq (contract-of previous-contract) active-contract) ERR_UPG_UNAUTHORIZED)
        (try! (contract-call? .token-owner-8742 finalize-ownership-transfer))
        (try! (contract-call? .ntt-manager-state-8742 finalize-ownership-transfer))
    ...
    (define-public (begin-state-transfer (successor principal))
      (let ((successor-parts (unwrap! (principal-destruct? successor) ERR_UPG_CHECK_CONTRACT_ADDRESS)))
        ...
        (try! (contract-call? .token-owner-8742 start-ownership-transfer successor))
        (try! (contract-call? .ntt-manager-state-8742 start-ownership-transfer successor))
        (ok true)))
    ...
    (define-public (cancel-update)
      (begin
        (try! (check-admin))
        (contract-call? .ntt-manager-state-8742 cancel-ownership-transfer)))
    
    ;;;; Read-only Functions
    
    (define-read-only (get-mode)
      (contract-call? .token-owner-8742 get-mode))
    
    (define-read-only (is-locking-mode)
      (contract-call? .token-owner-8742 is-locking-mode))
    
    (define-read-only (is-burning-mode)
      (contract-call? .token-owner-8742 is-burning-mode))
    ...
    (define-public (add-transceiver (transceiver <transceiver-trait>))
      (let ((protocol (try! (contract-call? transceiver get-protocol-id))))
        (try! (check-admin))
        (contract-call? .ntt-manager-state-8742 add-transceiver (contract-of transceiver) protocol)))
    ...
    (define-public (remove-transceiver (p principal))
      (begin
        (try! (check-admin))
        (contract-call? .ntt-manager-state-8742 remove-transceiver p)))
    ...
    (define-read-only (get-transceiver-trait-protocol (transceiver <transceiver-trait>))
      (contract-call? .ntt-manager-state-8742 transceivers-get (contract-of transceiver)))
    
    ;; @desc Check principal is a registered transceiver
    ;;       Returns `(ok protocol)` if so
    (define-read-only (check-transceiver (p principal))
      (ok (unwrap! (contract-call? .ntt-manager-state-8742 transceivers-get p) ERR_TRANSCEIVER_UNAUTHORIZED)))
