...
async fn test_governance() {
    ...
    let ix = native_token_transfers::instruction::TransferOwnership;

    let accs = native_token_transfers::accounts::TransferOwnership {
    ...
    let inner_ix_data = native_token_transfers::instruction::ClaimOwnership {};
    let inner_ix_accs = native_token_transfers::accounts::ClaimOwnership {
        new_owner: OWNER,
        config: test_data.ntt.config(),
        upgrade_lock: test_data.ntt.upgrade_lock(),
        program_data: test_data.ntt.program_data(),
...
async fn test_governance_one_step_transfer() {
    ...
    let ix = native_token_transfers::instruction::TransferOwnershipOneStepUnchecked;
