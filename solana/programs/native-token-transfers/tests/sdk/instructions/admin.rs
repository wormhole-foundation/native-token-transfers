...
use native_token_transfers::instructions::SetPeerArgs;
...
pub fn set_peer(ntt: &NTT, accounts: SetPeer, args: SetPeerArgs) -> Instruction {
    ...
    let data = native_token_transfers::instruction::SetPeer { args };

    let accounts = native_token_transfers::accounts::SetPeer {
        config: ntt.config(),
    ...
    Instruction {
        program_id: native_token_transfers::ID,
...
pub fn set_paused(ntt: &NTT, accounts: SetPaused, pause: bool) -> Instruction {
    let data = native_token_transfers::instruction::SetPaused { pause };

    let accounts = native_token_transfers::accounts::SetPaused {
        owner: accounts.owner,
        config: ntt.config(),
    };
...
pub fn register_transceiver(ntt: &NTT, accounts: RegisterTransceiver) -> Instruction {
    let data = native_token_transfers::instruction::RegisterTransceiver {};

    let accounts = native_token_transfers::accounts::RegisterTransceiver {
        config: ntt.config(),
        owner: accounts.owner,
        payer: accounts.payer,
...
pub fn deregister_transceiver(ntt: &NTT, accounts: DeregisterTransceiver) -> Instruction {
    let data = native_token_transfers::instruction::DeregisterTransceiver {};

    let accounts = native_token_transfers::accounts::DeregisterTransceiver {
        config: ntt.config(),
        owner: accounts.owner,
        transceiver: accounts.transceiver,
        registered_transceiver: ntt.registered_transceiver(&accounts.transceiver),
...
pub fn set_threshold(ntt: &NTT, accounts: SetThreshold, threshold: u8) -> Instruction {
    let data = native_token_transfers::instruction::SetThreshold { threshold };

    let accounts = native_token_transfers::accounts::SetThreshold {
        config: ntt.config(),
        owner: accounts.owner,
    };
