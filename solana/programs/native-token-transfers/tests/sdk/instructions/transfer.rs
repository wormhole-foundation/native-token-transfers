...
use native_token_transfers::{accounts::NotPausedConfig, instructions::TransferArgs};
...
pub fn transfer_burn_with_token_program_id(
    ...
    let data = native_token_transfers::instruction::TransferBurn { args };

    let accounts = native_token_transfers::accounts::TransferBurn {
...
pub fn transfer_lock_with_token_program_id(
    ...
    let chain_id = args.recipient_chain.id;
    let session_authority = ntt.session_authority(&transfer.from_authority, &args);
    let data = native_token_transfers::instruction::TransferLock { args };

    let accounts = native_token_transfers::accounts::TransferLock {
        common: common_with_token_program_id(ntt, &transfer, token_program_id),
        inbox_rate_limit: ntt.inbox_rate_limit(chain_id),
        peer: transfer.peer,
        session_authority,
...
fn common_with_token_program_id(
    ...
    transfer: &Transfer,
    token_program_id: &Pubkey,
) -> native_token_transfers::accounts::Transfer {
    native_token_transfers::accounts::Transfer {
