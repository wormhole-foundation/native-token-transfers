...
use native_token_transfers::instructions::RedeemArgs;
...
pub fn redeem(ntt: &NTT, accs: Redeem, args: RedeemArgs) -> Instruction {
    let data = native_token_transfers::instruction::Redeem { args };

    let accounts = native_token_transfers::accounts::Redeem {
        payer: accs.payer,
        config: ntt.config(),
        peer: accs.peer,
