...
pub fn receive_message(ntt: &NTT, accs: ReceiveMessage) -> Instruction {
    let data = native_token_transfers::instruction::ReceiveWormholeMessage {};

    let accounts = native_token_transfers::accounts::ReceiveMessage {
        payer: accs.payer,
        config: native_token_transfers::accounts::NotPausedConfig {
            config: ntt.config(),
        },
        peer: accs.peer,
        vaa: accs.vaa,
