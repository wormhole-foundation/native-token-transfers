...
use native_token_transfers::transceivers::wormhole::BroadcastPeerArgs;
...
pub fn broadcast_peer(ntt: &NTT, accs: BroadcastPeer) -> Instruction {
    let data = native_token_transfers::instruction::BroadcastWormholePeer {
        args: BroadcastPeerArgs {
            chain_id: accs.chain_id,
        },
    };

    let accounts = native_token_transfers::accounts::BroadcastPeer {
        payer: accs.payer,
        config: ntt.config(),
